...
def test_from_dict_classmethod():
    cfg = RootConfig.cfg.from_dict({"device": "cuda", "trainer": {"lr": 1e-4}})
...
def test_node_attachment_preserves_plain_output():
    ...
    assert NodeTree.cfg.from_dict(VALID_DATA).cfg.to_dict() == PlainTree.cfg.from_dict(VALID_DATA).cfg.to_dict()
...
def test_node_attachment_preserves_digest_and_serialized_forms():
    ...
    node_tree = NodeTree.cfg.from_dict(VALID_DATA)
    plain_tree = PlainTree.cfg.from_dict(VALID_DATA)
    assert node_tree.cfg.hash() == plain_tree.cfg.hash()
    assert node_tree.cfg.dumps_json() == plain_tree.cfg.dumps_json()
    assert node_tree.cfg.dumps_yaml() == plain_tree.cfg.dumps_yaml()
...
def test_node_attachment_preserves_issue_reports():
    ...
    with pytest.raises(ConfigError) as node_error:
        NodeTree.cfg.from_dict(INVALID_DATA, context="same source")
    with pytest.raises(ConfigError) as plain_error:
        PlainTree.cfg.from_dict(INVALID_DATA, context="same source")
...
def test_node_attachment_preserves_hook_counts():
    ...
    HOOK_CALLS.clear()
    NodeTree.cfg.from_dict(VALID_DATA)
    node_calls = list(HOOK_CALLS)

    HOOK_CALLS.clear()
    PlainTree.cfg.from_dict(VALID_DATA)
...
def test_node_attachment_preserves_schema_preflight_errors():
    ...
    with pytest.raises(ConfigError) as node_error:
        NodeBadTree.cfg.from_dict({})
    with pytest.raises(ConfigError) as plain_error:
        PlainBadTree.cfg.from_dict({})
...
def test_node_attachment_preserves_any_and_field_projections():
    ...
    assert NodeTree.cfg.from_dict({**VALID_DATA, "note": "other"}) == node_tree
    assert PlainTree.cfg.from_dict({**VALID_DATA, "note": "other"}) == plain_tree
    assert NodeTree.cfg.from_dict({**VALID_DATA, "weight": 9.0}).cfg.hash() == node_tree.cfg.hash()
    assert PlainTree.cfg.from_dict({**VALID_DATA, "weight": 9.0}).cfg.hash() == plain_tree.cfg.hash()
...
def test_nested_node_methods_match_free_functions():
    cfg = NodeTree.cfg.from_dict(VALID_DATA)
    assert cfg.mid.leaf.cfg.to_dict() == to_dict(cfg.mid.leaf)
    assert cfg.mid.leaf.cfg.hash() == config_hash(cfg.mid.leaf)
    assert cfg.mid.cfg.dumps_json() == dumps_json(cfg.mid)
    assert cfg.mid.cfg.dumps_yaml() == dumps_yaml(cfg.mid)


def test_nested_node_round_trips_through_its_own_class():
    cfg = NodeTree.cfg.from_dict(VALID_DATA)
    assert NodeLeaf.cfg.from_dict(cfg.mid.leaf.cfg.to_dict()) == cfg.mid.leaf
    assert NodeMid.cfg.from_dict(cfg.mid.cfg.to_dict()) == cfg.mid


@pytest.mark.parametrize("suffix", [".json", ".yaml", ".yml"])
def test_nested_node_file_round_trip(tmp_path: Path, suffix: str):
    cfg = NodeTree.cfg.from_dict(VALID_DATA)
    path = cfg.mid.cfg.to_file(tmp_path / f"mid{suffix}")
    assert NodeMid.cfg.from_file(path) == cfg.mid


def test_nested_snapshot_loaded_through_enclosing_class_reports_its_keys(tmp_path: Path):
    cfg = NodeTree.cfg.from_dict(VALID_DATA)
    path = cfg.mid.cfg.save_json(tmp_path / "mid.json")
    with pytest.raises(ConfigError) as error:
        NodeTree.cfg.load_json(path)
...
def test_issue_paths_are_relative_to_the_entry_node():
    with pytest.raises(ConfigError) as outer:
        NodeTree.cfg.from_dict({"mid": {"leaf": {"name": "adamw", "lr": "fast"}}})
    with pytest.raises(ConfigError) as inner:
        NodeLeaf.cfg.from_dict({"name": "adamw", "lr": "fast"})
...
def test_nested_node_entry_keeps_the_callers_context():
    with pytest.raises(ConfigError) as error:
        NodeLeaf.cfg.from_dict({"lr": 1.0}, context="optimizer override")
...
def test_enclosing_hash_excludes_an_unhashed_section():
    ...
    assert left.cfg.hash() == right.cfg.hash()
    assert left.inner.cfg.hash() != right.inner.cfg.hash()
...
def test_identical_projections_share_a_digest_across_classes():
    ...
    assert leaf.cfg.hash() == twin.cfg.hash()
...
def test_schema_cache_warm_order_does_not_change_reports():
    ...
    def build() -> tuple[Any, list[tuple[str, str]]]:
        built = NodeTree.cfg.from_dict(VALID_DATA).cfg.to_dict()
        with pytest.raises(ConfigError) as error:
            NodeTree.cfg.from_dict(INVALID_DATA)
...
def test_node_subclassing_a_node_stays_a_node():
    derived = NodeDerived.cfg.from_dict({"a": 5})
    assert derived == NodeDerived(a=5, b=2)
    assert derived.cfg.to_dict() == {"a": 5, "b": 2}
    assert derived.cfg.hash() == config_hash(derived)
